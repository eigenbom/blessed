...
function Tput(options) {
  ...
  this.usrPath = options.usrPath || path.join(__dirname, '..', 'usr');
  this.error = null;
...
Tput.prototype._useXtermCap = function() {
  return this.injectTermcap(path.join(this.usrPath, 'xterm.termcap'));
};

Tput.prototype._useXtermInfo = function() {
  return this.injectTerminfo(path.join(this.usrPath,  'xterm'));
};

Tput.prototype._useInternalInfo = function(name) {
  name = path.basename(name);
  return this.injectTerminfo(path.join(this.usrPath, name));
};

Tput.prototype._useInternalCap = function(name) {
  name = path.basename(name);
  return this.injectTermcap(path.join(this.usrPath, name + '.termcap'));
};
